...
public class FTPClient implements FTPCommands {
     ...
     *
     * @param ispassive true to make the connection established with passive mode.
     ...
     *
     * @param isSecured true to set all connection secured; false to be not secured.
     */
     ...
     *
     * @param serverResponse Full response message from the server.
...
//System.out.println("d");
    ...
    public int search(String want) throws IOException {
        establishDataLink();
        send(LIST);
        // While we are waiting for this respond, server is connecting our port.
...
//System.out.println("d");
            if (split.length < 9) {
                ...
                System.out.println(filename.toString() + "  " + Integer.valueOf(s));
                hm.put(filename.toString(), Integer.valueOf(s));
     ...
     *
     * @param dir name of the directory to switch to. Must be the sub directory
     ...
     *
     * @param filename Name of the file only. It must be under the current working
     *                 directory.
     * @param localDir URL of local file system directory, where to save the file.
     ...
     *
     * @param filepath Full path of the local filepath to upload.
     * @throws IOException
     */
    public boolean store(String filepath) throws IOException {
        ...
        //if (hm.containsKey(want))
        nu = (int) search(file.getName());
        establishDataLink();

        // set transfer mode to binary
        send(TYPE + " I");
        read();
        send(APPE + " " + file.getName());
        // System.out.println(read());
     ...
     * Rename a file.
     *
     * @param srcfile Original file name.
     * @param dstfile The file name to rename to.
     * @return true if this command is successful; otherwise false.
     * @throws IOException
     */
     ...
     * read them all.
     *
     * @param oneline read only one line or multiple lines.
